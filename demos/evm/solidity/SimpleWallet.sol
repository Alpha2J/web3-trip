...
contract SimpleWallet {
    ...
    // Software applications can listen for events in order to react to solidity state changes.
    event LogDeposit(uint amount, address indexed sender);
    event LogWithdrawal(uint amount, address indexed recipient);

	// When this solidity is deployed, set the deploying address.sol as the owner of the solidity.
    constructor() {
        owner = payable(msg.sender);
    }

    // Send ETH from the function caller to the SimpleWallet solidity
    function deposit() public payable {
        require(msg.value > 0, "Must send ETH.");
        emit LogDeposit(msg.value, msg.sender);
    }

    // Send ETH from the SimpleWallet solidity to a chosen recipient
